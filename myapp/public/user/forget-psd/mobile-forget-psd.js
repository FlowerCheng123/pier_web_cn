...
.factory('SdkUrl', function(){
	var hostName = '';
	return{
		getValidCode: hostName+ '/sdk/v1/users/sdkSMSPsdForget',
		activation: hostName+ '/sdk/v1/users/sdkSMSPsdCheck',
		resetPsd: hostName+ '/sdk/v1/users/sdkResetPsd'
	}
})
.controller( 'ForgetController',function($scope,$log, HttpService, SdkUrl, $timeout){
    ...
    $scope.verifyId = function(){
    	...
    	pNext.then( function(result){
    		...
    		window.location.href="/users/resetPassword/"+result.token+"?action=10012";
...
.controller( 'ResetController', function( $scope, $log, HttpService, SdkUrl){
	...
	$scope.resetPsd = function(){
		...
		pReset.then(function(result){
			...
			window.location.href="/users/resetSuccess?action=10013";
